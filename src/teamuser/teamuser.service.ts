...
import { TeamUserRepository } from 'src/teamuser/teamuser.repository';


@Injectable()
export class TeamUserService extends BaseService<TeamUser> {
  constructor(
    @InjectRepository(TeamUser) protected repository: Repository<TeamUser>,
    protected delivery: TeamUserService,
    protected gardener: TeamUserRepository,
    protected requests: TeamUserRepository
  ) {
    super(repository);
  }
   ...
   */
  override async update(id: number, updates: Partial<TeamUser>) {
  ...
  *     same batch/harvest.
  */
  /* async create(data: Omit<TeamUser, 'id'>) {
